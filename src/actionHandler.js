...
domCreator.eventEmitter.on("newProject", (projectName, projectLabelID) => {
  const projectLabel = getLabelList().filter(
    (label) => label.id === projectLabelID
  )[0];
...
domCreator.eventEmitter.on("taskListWeek", function () {
  ...
  const todayTasks = getFullTaskList().filter((task) =>
...
domCreator.eventEmitter.on("taskListImportant", function () {
  ...
  const highPrioTasks = getFullTaskList().filter(
...
domCreator.eventEmitter.on("sortTasks", (sorter, nodes, isDescending) => {
  ...
  if (sorter === "Priority") {
    tasksToSort = tasksToSort.sort((a, b) => {
      let aPrio = a.priority === "high" ? 3 : a.priority === "medium" ? 2 : 1;
      let bPrio = b.priority === "high" ? 3 : b.priority === "medium" ? 2 : 1;
      return aPrio - bPrio;
    });
  }
  if (isDescending) {
...
domCreator.eventEmitter.on(
  "newTask",
  (projectID, taskTitle, taskDesc, taskDueDate, taskPriority) => {
    const newTaskObj = {
      title: taskTitle,
    ...
    if (projectID === Number(getCurrentProject())) {
      domCreator.taskContainer.appendChild(
        domCreator.drawTask(newTask, project)
      );
    }
    saveToLocal();
  }
...
domCreator.eventEmitter.on(
  "editTask",
  (projectID, taskTitle, taskDesc, taskDueDate, taskPriority, task) => {
    const project = getProjectByID(projectID);
...
domCreator.eventEmitter.on(
  "editProject",
  (projectTitle, projectLabelID, project) => {
...
domCreator.eventEmitter.on(
  "labelsEditPopup",
  (selectInputContainer, project) => {
    const labelList = getLabelList();
    domCreator.drawEditLabelsContainer(
      labelList,
...
if (!localStorage.getItem("projectList")) {
  ...
  setLabelID(4);
  const taskObjectCreator = function (
    title,
    description,
    dueDate,
    completed,
  ...
  let myTask = new Task(
    taskObjectCreator(
      ...
      "The ski season is coming, and it's coming hard",
      addDays(new Date(), 4),
      false,
      "low"
    )
  ...
  let myTask2 = new Task(
    taskObjectCreator(
      ...
      "It's always a doozy. Also, use less electricity",
      addDays(new Date(), 1),
      false,
      "high"
    )
  );
  let myTask3 = new Task(
    taskObjectCreator(
      "Buy a new hat",
      "Thinking cyan, but also open to magenta, yellow or black",
      false,
      false,
      "medium"
    )
  );
  let myTask4 = new Task(
    taskObjectCreator(
      "Create an example task",
      "To pad out the website upon a user's initial load, so they can see the options and functionality",
      new Date(),
      true,
      "medium"
    )
  );
  let myTask5 = new Task(
    taskObjectCreator(
      "Add hidden reset button",
      "Need to have a way for user to reset entire list, and hide it in the header title period",
      false,
      true,
      "medium"
    )
  );
  let myTask6 = new Task(
    taskObjectCreator(
      "Implement localStorage",
      "Needed in order for tasks to be saved to user's computer, and to remember their preferences in the app",
      subDays(new Date(), 6),
      true,
      "high"
    )
  );
  let myTask7 = new Task(
    taskObjectCreator(
      "Add dark mode",
      "Make it toggle via a  visual button, and detect browser's default preference for initial toggle",
      new Date(),
      true,
      "low"
    )
  ...
  let myTask9 = new Task(
    taskObjectCreator("Bread", "", false, false, "medium")
  );
  let myTask10 = new Task(
    taskObjectCreator("Pistachio Nuts", "", new Date(), false, "high")
  );
